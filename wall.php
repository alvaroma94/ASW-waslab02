...
switch ($_SERVER['REQUEST_METHOD']) {
	...
	case 'POST': 
		...
		$autor = $_POST["author"];
		$tweet = $_POST["tweet_text"];
		$dbhandler->insertTweet($autor,$tweet);
		break;
	...
	case 'PUT':
        ...
        $autor = $tweet->author;
        $texto = $tweet->text;
        $dbhandler->insertTweet($autor,$texto);
	...
	case 'DELETE':
        ...
        if($result){
			echo '<?xml version="1.0"?>
			<response><deletion tweetid="13">success</deletion></response>';
        }
        else{
            echo '<?xml version="1.0"?>
			<response><deletion tweetid="13">fail</deletion></response>';
            }
